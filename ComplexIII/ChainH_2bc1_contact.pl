...
use warnings;
 ...
 # This script reads the PDB file (1BE3) for bovine complex III (bc1.pdb2 complex). This structure consist of 22 chains, being chain H the corresponding to subunit 6
 # We want to sort each Chain H residues into two sects. Contact set includes those residues from Chain H that are in close contact (<= 4 Angstroms) with residues from
 ...
 # residues from Chain H that are in close contact with other subunits (Contact_2bc1_set.txt).
...
open (OUTPUT, ">ChainH_Detailed_2bc1_Results.txt") or die print "Couldn't write the file \n";
open (CONTACT, ">ChainH_Contact_2bc1_Set.txt") or die print "Couldn't at 121 \n";
...
for ($i=0; $i< 524; ++$i) {
      ...
      for ($j = 0; $j < 3458; ++$j) { # distances to atoms from chain A
            $distance = sqrt( ($X_chainH[$i]-$X_chainA[$j])**2 + ($Y_chainH[$i]-$Y_chainA[$j])**2 + ($Z_chainH[$i]-$Z_chainA[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainA = split (/\s+/, $row[$j]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainA[3]." ".$fromchainA[5], "    Chain A (Core 1) \n";
            }
      }

      for ($j = 0; $j < 3141; ++$j) { # distances to atoms from chain B
            $distance = sqrt( ($X_chainH[$i]-$X_chainB[$j])**2 + ($Y_chainH[$i]-$Y_chainB[$j])**2 + ($Z_chainH[$i]-$Z_chainB[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+3459], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainB = split (/\s+/, $row[$j+3459]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainB[3].$fromchainB[5], "    Chain B (Core 2) \n";
            }

      }

      for ($j = 0; $j < 3011; ++$j) { # distances to atoms from chain C
            $distance = sqrt( ($X_chainH[$i]-$X_chainC[$j])**2 + ($Y_chainH[$i]-$Y_chainC[$j])**2 + ($Z_chainH[$i]-$Z_chainC[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+6601], "Distance =", $distance, "\n\n";
                  my @fromchainC = split (/\s+/, $row[$j+6601]);
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainC[3].$fromchainC[5], "    Chain C (Cytb) \n";
            }
      }
      
      for ($j = 0; $j < 1919; ++$j) { # distances to atoms from chain D
            $distance = sqrt( ($X_chainH[$i]-$X_chainD[$j])**2 + ($Y_chainH[$i]-$Y_chainD[$j])**2 + ($Z_chainH[$i]-$Z_chainD[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+9613], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainD = split (/\s+/, $row[$j+9613]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainD[3].$fromchainD[5], "    Chain D (Cytc1) \n";
            }
      }
      for ($j=0; $j< 1519; ++$j) { # distances to atoms from chain E
            $distance = sqrt( ($X_chainH[$i]-$X_chainE[$j])**2 + ($Y_chainH[$i]-$Y_chainE[$j])**2 + ($Z_chainH[$i]-$Z_chainE[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+11533], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainE = split (/\s+/, $row[$j+11533]);
                  print CONTACT $fromchainH[5]."  ".$fromchainH[3], "\t", $fromchainE[3].$fromchainE[5], "    Chain E (ISP) \n";
            }
      }

      for ($j=0; $j< 916; ++$j) { # distances to atoms from chain F
            $distance = sqrt( ($X_chainH[$i]-$X_chainF[$j])**2 + ($Y_chainH[$i]-$Y_chainF[$j])**2 + ($Z_chainH[$i]-$Z_chainF[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+13053], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainF = split (/\s+/, $row[$j+13053]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainF[3].$fromchainF[5], "    Chain F (Subunit 7) \n";
            }
      }
      
      for ($j=0; $j< 682; ++$j) { # distances to atoms from chain G
            $distance = sqrt( ($X_chainH[$i]-$X_chainG[$j])**2 + ($Y_chainH[$i]-$Y_chainG[$j])**2 + ($Z_chainH[$i]-$Z_chainG[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+13970], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainG = split (/\s+/, $row[$j+13970]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainG[3].$fromchainG[5], "    Chain G (Subunit 8) \n";
            }
      }


      for ($j=0; $j< 248; ++$j) { # distances to atoms from chain I
            $distance = sqrt( ($X_chainH[$i]-$X_chainI[$j])**2 + ($Y_chainH[$i]-$Y_chainI[$j])**2 + ($Z_chainH[$i]-$Z_chainI[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+15178], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainI = split (/\s+/, $row[$j+15178]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainI[3].$fromchainI[5], "    Chain I (Subunit 11?) \n";
            }
      }
      
      for ($j=0; $j< 512; ++$j) { # distances to atoms from chain J
            $distance = sqrt( ($X_chainH[$i]-$X_chainJ[$j])**2 + ($Y_chainH[$i]-$Y_chainJ[$j])**2 + ($Z_chainH[$i]-$Z_chainJ[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+15427], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainJ = split (/\s+/, $row[$j+15427]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainJ[3].$fromchainJ[5], "    Chain J (Subunit 9) \n";
            }
      }
      
      for ($j=0; $j< 159; ++$j) { # distances to atoms from chain K
            $distance = sqrt( ($X_chainH[$i]-$X_chainK[$j])**2 + ($Y_chainH[$i]-$Y_chainK[$j])**2 + ($Z_chainH[$i]-$Z_chainK[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+15940], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainK = split (/\s+/, $row[$j+15940]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainK[3].$fromchainK[5], "    Chain K (Subunit 10) \n";
            }
      }
      
      
      for ($j = 0; $j < 3458; ++$j) { # distances to atoms from chain A2
            $distance = sqrt( ($X_chainH[$i]-$X_chainAA[$j])**2 + ($Y_chainH[$i]-$Y_chainAA[$j])**2 + ($Z_chainH[$i]-$Z_chainAA[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+16100], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainAA = split (/\s+/, $row[$j+16100]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainAA[3].$fromchainAA[5], "    Chain A2 (Core 2) \n";
            }

      }
      
      for ($j = 0; $j < 3141; ++$j) { # distances to atoms from chain B2
            $distance = sqrt( ($X_chainH[$i]-$X_chainBB[$j])**2 + ($Y_chainH[$i]-$Y_chainBB[$j])**2 + ($Z_chainH[$i]-$Z_chainBB[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+19559], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainBB = split (/\s+/, $row[$j+19559]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainBB[3].$fromchainBB[5], "    Chain B2 (Core 2) \n";
            }

      }

      for ($j = 0; $j < 3011; ++$j) { # distances to atoms from chain C2
            $distance = sqrt( ($X_chainH[$i]-$X_chainCC[$j])**2 + ($Y_chainH[$i]-$Y_chainCC[$j])**2 + ($Z_chainH[$i]-$Z_chainCC[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+22701], "Distance =", $distance, "\n\n";
                  my @fromchainCC = split (/\s+/, $row[$j+22701]);
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainCC[3].$fromchainCC[5], "    Chain C2 (Cytb) \n";
            }
      }
      
      for ($j = 0; $j < 1919; ++$j) { # distances to atoms from chain D2
            $distance = sqrt( ($X_chainH[$i]-$X_chainDD[$j])**2 + ($Y_chainH[$i]-$Y_chainDD[$j])**2 + ($Z_chainH[$i]-$Z_chainDD[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+25713], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainDD = split (/\s+/, $row[$j+25713]);
                  print CONTACT  $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainDD[3].$fromchainDD[5], "    Chain D2 (Cytc1) \n";
            }
      }
      
      for ($j=0; $j< 1519; ++$j) { # distances to atoms from chain E2
            $distance = sqrt( ($X_chainH[$i]-$X_chainEE[$j])**2 + ($Y_chainH[$i]-$Y_chainEE[$j])**2 + ($Z_chainH[$i]-$Z_chainEE[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+27633], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainEE = split (/\s+/, $row[$j+27633]);
                  print CONTACT $fromchainH[5]."  ".$fromchainH[3], "\t", $fromchainEE[3].$fromchainEE[5], "    Chain E2 (ISP) \n";
            }
      }

      for ($j=0; $j< 916; ++$j) { # distances to atoms from chain F2
            $distance = sqrt( ($X_chainH[$i]-$X_chainFF[$j])**2 + ($Y_chainH[$i]-$Y_chainFF[$j])**2 + ($Z_chainH[$i]-$Z_chainFF[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+29153], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainFF = split (/\s+/, $row[$j+29153]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainFF[3].$fromchainFF[5], "    Chain F2 (Subunit 7) \n";
            }
      }
      
      for ($j=0; $j< 682; ++$j) { # distances to atoms from chain G2
            $distance = sqrt( ($X_chainH[$i]-$X_chainGG[$j])**2 + ($Y_chainH[$i]-$Y_chainGG[$j])**2 + ($Z_chainH[$i]-$Z_chainGG[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+30070], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainGG = split (/\s+/, $row[$j+30070]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainGG[3].$fromchainGG[5], "    Chain G2 (Subunit 8) \n";
            }
      }

      for ($j=0; $j< 524; ++$j) { # distances to atoms from chain H2
            $distance = sqrt( ($X_chainH[$i]-$X_chainHH[$j])**2 + ($Y_chainH[$i]-$Y_chainHH[$j])**2 + ($Z_chainH[$i]-$Z_chainHH[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+30753], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainHH = split (/\s+/, $row[$j+30753]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainHH[3].$fromchainHH[5], "    Chain H2 (Subunit 6) \n";
            }
      }

      for ($j=0; $j< 248; ++$j) { # distances to atoms from chain I2
            $distance = sqrt( ($X_chainH[$i]-$X_chainII[$j])**2 + ($Y_chainH[$i]-$Y_chainII[$j])**2 + ($Z_chainH[$i]-$Z_chainII[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+31278], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainII = split (/\s+/, $row[$j+31278]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainII[3].$fromchainII[5], "    Chain I2 (Subunit 11?) \n";
            }
      }
      
      for ($j=0; $j< 512; ++$j) { # distances to atoms from chain J2
            $distance = sqrt( ($X_chainH[$i]-$X_chainJJ[$j])**2 + ($Y_chainH[$i]-$Y_chainJJ[$j])**2 + ($Z_chainH[$i]-$Z_chainJJ[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+31527], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainJJ = split (/\s+/, $row[$j+31527]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainJJ[3].$fromchainJJ[5], "    Chain J2 (Subunit 9) \n";
            }
      }
      
      for ($j=0; $j< 159; ++$j) { # distances to atoms from chain K2
            $distance = sqrt( ($X_chainH[$i]-$X_chainKK[$j])**2 + ($Y_chainH[$i]-$Y_chainKK[$j])**2 + ($Z_chainH[$i]-$Z_chainKK[$j])**2 );
            if ($distance <= 4) {
                  print OUTPUT $row[$i+14653], $row[$j+32040], "Distance =", $distance, "\n\n";
                  my @fromchainH = split (/\s+/, $row[$i+14653]);
                  my @fromchainKK = split (/\s+/, $row[$j+32040]);
                  print CONTACT $fromchainH[5]." ".$fromchainH[3], "\t", $fromchainKK[3].$fromchainKK[5], "    Chain K2 (Subunit 10) \n";
            }
      }
...
open (FILE, "ChainH_Contact_2bc1_Set.txt") or die print "Couldn't at 447 \n";
my @array = <FILE>;
close FILE;
...
open (OUTPUT, ">ChainH_Contact_2bc1_Set.txt") or die print "Couldn't at 454 \n";
